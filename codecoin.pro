...
# use: qmake "COIN_BRAND=mannastake" for other than manna
contains(COIN_BRAND, manna) {
    message(Building for Manna)
    DEFINES += BRAND_manna
    TARGET = manna
} else {
contains(COIN_BRAND, mannastake) {
    message(Building for Mannastake)
    DEFINES += BRAND_mannastake
    STAKE = 1
    TARGET = mannastake
}}
