...
// manna: optional setting to unlock wallet for block minting only;
...
#if defined(PPCOINSTAKE)
// manna: total coins staked (non-spendable until maturity)
...
//#endif
{
	...
	if (pcoin->nTime > nSpendTime)
		continue; // manna: timestamp must not exceed spend time
#endif
...
#if defined(PPCOINSTAKE)
// manna: create coin stake transaction
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, CTransaction& txNew)
{
     ...
     * working theory is with manna PoW seigniorage being inversely related
...
#if defined(PPCOINSTAKE)
// manna: check 'spent' consistency between wallet and txindex
// manna: fix wallet spent state according to txindex
...
// manna: disable transaction (only for coinstake)
