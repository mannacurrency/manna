...
string FormatMoney(int64_t n, bool fPlus)
{
    ...
    string str = strprintf("%" PRId64".%" COIN_DECIMALS_FMT "" PRId64 "", quotient, remainder);
...
void AddTimeData(const CNetAddr& ip, int64_t nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        if (abs64(nMedian) < 5 * 60) // Catcoin: changed maximum adjust to 5 mins to avoid letting peers change our time too much in case of an attack.
        {
            nTimeOffset = nMedian;
        }
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong " BRAND_upper " will not work properly.");
