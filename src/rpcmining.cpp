...
Value getestnextdiff(const Array& params, bool fHelp)
{
	int64_t timestamp = 0;
	if (fHelp || params.size() > 1)
		throw runtime_error(
			"getestnextdiff [timestamp]\n"
			"Returns the estimated next difficulty if a block were found at [timestamp]\n"
			"or now if no timestamp");

	if (params.size() > 0) timestamp = params[0].get_int();
