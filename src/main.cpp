...
void SyncWithWallets(const uint256 &hash, const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
	if (!fConnect)
	{
		// manna: wallets need to refund inputs when disconnecting coinstake
		if (tx.IsCoinStake())
		{
			BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
...
{
	// Codecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
#if defined(PPCOINSTAKE)
// manna: find block wanted by given orphan block
...
// manna: find last block index up to pindex
...
bool CTransaction::CheckInputs(CValidationState &state, CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags, std::vector<CScriptCheck> *pvChecks) const
{
	...
	#warning "can't check previous tx timestamps without CCoins mannastake structures"
#endif
		...
		if (IsCoinStake())
		{
			// manna: coin stake tx earns reward instead of paying fee
			uint64_t nCoinAge;
...
#endif
		nFees += nTxFee; // TODO: check this with manna logic. May not be correct for PPCOINSTAKE
		if (!MoneyRange(nFees))
...
void ThreadScriptCheck() {
	RenameThread("codecoin-scriptch");
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
	...
	// BIP16 didn't become active until Oct 1 2012	 FIXME: 0xdeadc0de
...
#if defined(PPCOINSTAKE)
    // manna: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;

    // manna: fees are not collected by miners as in bitcoin
    // manna: fees are destroyed to compensate the entire network
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
	...
	nTransactionsUpdated++;
	printf("SetBestChain: new best=%s  height=%d  log2_trust=%.8g  tx=%lu  date=%s progress=%f\n",
...
#if defined(PPCOINSTAKE)
// manna: total coin age spent in transaction, in the unit of coin-days.
...
// manna: total coin age spent in block, in the unit of coin-days.
...
#if defined(PPCOINSTAKE)
	// manna: compute stake entropy bit for stake modifier
	if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
		return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

	// manna: record proof-of-stake hash value
...
#if defined(PPCOINSTAKE)
	// manna: only the second transaction can be the optional coinstake
	for (unsigned int i = 2; i < vtx.size(); i++)
		if (vtx[i].IsCoinStake())
			return state.DoS(100, error("CheckBlock() : coinstake in wrong position"));

	// manna: coinbase output should be empty if proof-of-stake block
...
#else
#warning "extra manna ProofOfWork reward checks disabled!!"
#endif
#endif
	...
	BOOST_FOREACH(const CTransaction& tx, vtx)
	{
		...
		// manna: check transaction timestamp
	...
	// Check for duplicate txids. This is caught by CheckInputs(),
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
	...
	if (hash != hashGenesisBlock) {
		...
		// manna: check that the block satisfies synchronized checkpoint
...
#if defined(SYNC_CHECKPOINTS)
	// manna: check pending sync-checkpoint
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
...
#if defined(PPCOINSTAKE)
	// manna: verify hash target and signature of coinstake tx
	if (pblock->IsProofOfStake())
	{
	...
	CBlockIndex* pcheckpoint = Checkpoints::GetLastCheckpoint(mapBlockIndex); /* regular hardened checkpoints */
	if (pcheckpoint && pblock->hashPrevBlock != hashBestChain)
	{
		...
		// https://bitbucket.org/tmagik/catoshi/issue/14/add-a-threshold-for-orphans-with-slightly
		CBigNum bnNewBlock;
...
#if defined(PPCOINSTAKE)
	// manna: ask for pending sync-checkpoint if any
...
#if defined(PPCOINSTAKE)
			// manna: getblocks may not obtain the ancestor block rejected
...
#if defined(SYNC_CHECKPOINTS)
	// manna: if responsible for sync-checkpoint send it
...
// manna: entropy bit for stake modifier if chosen by modifier
...
string GetWarnings(string strFor)
{
...
#if defined(PPCOINSTAKE)
	// manna: wallet lock warning for minting
	if (strMintWarning != "")
	{
		nPriority = 0;
...
#if defined(SYNC_CHECKPOINTS)
	// manna: should not enter safe mode for longer invalid chain
	// manna: if sync-checkpoint is too old do not enter safe mode
	...
	// manna: if detected invalid checkpoint enter safe mode
	...
	// Alerts
	{
		...
		BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
		{
			...
			if (alert.AppliesToMe() && alert.nPriority > nPriority)
			{
				...
				if (nPriority > 1000)
					strRPC = strStatusBar;	// manna: safe mode for high alert
			}
		}
	}
#endif /* ENABLE_ALERTS */
...
#if defined(PPCOINSTAKE)
		// manna: record my external IP reported by peer
...
#else
		printf("Unknown alert! TODO: add debug print!\n");
		pfrom->Misbehaving(1);
#endif /* ENABLE_ALERTS */
	}
...
#if defined(PPCOINSTAKE)
	// manna: if coinstake available add coinstake tx
...
#if defined(PPCOINSTAKE) || defined(BRAND_manna)
			// manna: simplify transaction fee - allow free = false
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
	...
	if (hash > hashTarget && pblock->IsProofOfWork())
		return error("CodecoinMiner : proof-of-work not meeting target");

	//// debug print
	printf("CodecoinMiner:\n");
	...
	// Found a solution
	{
		...
		if (pblock->hashPrevBlock != hashBestChain)
			return error("CodecoinMiner : generated block is stale");
		...
		if (!ProcessBlock(state, NULL, pblock))
			return error("CodecoinMiner : ProcessBlock, block not accepted");
	}

	return true;
}

// TODO: move this miner stuff to miner.cpp
void CodecoinMiner(CWallet *pwallet, bool fProofOfStake)
{
	printf("CodecoinMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("codecoin-miner");
	...
	try { while (true) {
		...
		if (fProofOfStake)
		{
			// manna: if proof-of-stake block found then process block
			if (pblock->IsProofOfStake())
			{
...
#endif
		printf("Running CodeCoinMiner with %" PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
#if defined(PPCOINSTAKE) || defined(BRAND_manna) /* double update? how does that work? */
...
//			#warning little-endian-ism
	...
	{
		printf("CodecoinMiner terminated\n");
		throw;
	}
}
